...
public class ReferenceTypeAccountArrayMain {
	...
	public static void main(String[] args) {
		...
		for(int i=0;i<accounts.length;i++) {
			/*
			 * length를 붙이는 이유는 accounts의 값만큼만 반복을 시키기 위해서 붙인다.
			 */
			/*
...
//			totBalance=totBalance + accounts[i].getBalance();
		...
		System.out.println("6.은행 계좌들중에서 이름이 KIM인 계좌 여러개 찾아서 출력[String객체 배우고난후에]");
		...
		System.out.println("9.계좌이름순으로 오름(내림)차순정렬[Quiz]");
		System.out.println("9.계좌이름순으로 오름(내림)차순정렬(String 클래스를 배운후에 추가)[Quiz]");
		...
		System.out.println("--------Account에 재정의된 toString 메쏘드--------------");
		for(Account account:accounts)
			System.out.println(account);/*(account.toString) .toString이 삭제 되어있다.*/
