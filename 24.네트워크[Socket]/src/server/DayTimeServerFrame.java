...
public class DayTimeServerFrame extends JFrame {
	...
	*/
	public class DayTimeServerThread extends Thread{
		...
		public void run() {
			try {
			displayLog("0.DayTimeServerThread:ServerSoket 생성(8000)");	
			...
			while(true) {
				...
				out.println("승근PC Server time"+serverTimeStr);
				out.flush();
				displayLog("4.DayTimeServerThread:소켓으로부터 생성된 OutputStream에서 서버쓰기");	
				socket.close();
				displayLog("5.DayTimeServerThread:클라이언트연결요청대기(8000)");	
			}
