 ...
 * 			2-1. 객체 필드(변수):인스턴스 생성되어야 사용할수있는 필드-(만들어질때마다 만들어지는 객체)
 *          2-2. 정적 필드(변수):인스턴스 생성과 관계없이 사용가능한 필드
 *                               클래스로딩시 클래스에 생성되는 필드(변수)-(클라스에 소속되어있기 때문에 객체의 영향을 받지 않는다. 자신만의 속성을 따름)
 *                               
 * - 메쏘드
 *  1.멤버메쏘드
 *          1-1.객체 메쏘드:인스턴스 생성되어야 호출가능한메쏘드- (참조변수가 있어야 한다.)
 *          1-2.정적 메쏘드:인스턴스 생성과 관계없이 호출가능한 메쏘드
 *                          클래스로딩시 클래스에 생성되는 메쏘드 (주소값이 필요없다.클라스에 소속되어있음.)                             
 */
