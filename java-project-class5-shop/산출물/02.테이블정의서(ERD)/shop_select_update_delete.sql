...
--로그인한멤버(guard1)의 카트 1개 아이템 수량변경
update cart set cart_qty = 3 where cart_no=8;

--로그인한멤버(guard1)의 카트에 존재하는 제품의 수(제품존재여부판단)
select count(*) cnt from cart where userid='guard1' and p_no=3;
...
select * from orders where userid='guard1';
--1.멤버한사람의 주문(주문아이템+제품정보)전체목록(guard1)
...
--2.멤버한 사람의 주문(주문아이템+제품)한개(guard1)
